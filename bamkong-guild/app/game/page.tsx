...
import { ArrowLeft, Droplet, Sun, Heart, Sparkles, Zap, Gamepad2, Gift, Home } from 'lucide-react';
...
export default function GamePage() {
  const { 
    ...
    gainExp, saveEvolution, isEvolved, evolutionId
  } = useBamkongGrowth();
  ...
  useEffect(() => {
    if (!isLoading) setDisplayLevel(level);
  }, [isLoading, level]);
  ...
  return (
    <div className="min-h-screen lg:h-screen lg:overflow-hidden flex flex-col font-sans selection:bg-amber-200 overflow-x-hidden relative text-stone-800">
        ...
        <div className="flex items-center gap-3">
          {user && (
            <div className="flex items-center gap-1.5 sm:gap-2 bg-white/80 backdrop-blur-md pl-1.5 sm:pl-2 pr-3 sm:pr-4 py-1.5 rounded-full border border-stone-200 shadow-sm max-w-[140px] sm:max-w-none">
              <img src={user.image} alt="프로필" className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border border-stone-200 object-cover bg-stone-100 shrink-0" />
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto flex flex-col lg:flex-row items-center justify-center gap-6 sm:gap-10 px-4 sm:px-6 pt-24 pb-8 z-10 overflow-y-auto lg:overflow-visible">
        <div className="w-full flex flex-col items-center flex-1 relative z-10">
          ...
          <div className="w-full max-w-xl bg-white/90 backdrop-blur-lg border border-amber-200/50 p-5 sm:p-7 rounded-[2rem] sm:rounded-[2.5rem] shadow-xl mb-8 sm:mb-10 flex flex-col gap-4 sm:gap-5 transform-gpu relative overflow-hidden">
            ...
            {level < 200 && (
              <div className="w-full h-4 sm:h-6 bg-stone-200/50 rounded-full overflow-hidden shadow-inner border border-stone-300/30">
                <div className="h-full bg-gradient-to-r from-amber-400 via-orange-400 to-amber-400 bg-[length:200%_auto] animate-gradient transition-all duration-500 ease-out transform-gpu" style={{ width: `${(exp / maxExp) * 100}%` }}></div>
              </div>
            )}
          </div>
          ...
          <div className="relative w-full max-w-[16rem] sm:max-w-sm aspect-square bg-white border border-amber-50 rounded-[2.5rem] sm:rounded-[3rem] shadow-xl flex flex-col items-center justify-center mb-8 sm:mb-10 overflow-hidden">
            ...
            {showEvolutionEffect && (
              <div className="absolute inset-0 z-40 flex items-center justify-center bg-white/50 backdrop-blur-[2px] animate-[fadeIn_0.2s_ease-out]">
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-amber-600 drop-shadow-md animate-[bounce_0.5s_ease-in-out_infinite]">성장중...</h1>
              </div>
            )}

            {actionType === 'water' && (
              <div className="absolute top-4 right-6 sm:right-8 z-30 flex flex-col items-center animate-[fadeIn_0.2s_ease-out]">
                <img src="/images/water-can.png" alt="물 주기" className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 object-contain drop-shadow-lg -rotate-48 scale-x-[-1] mb-1" />
            ...
            {actionType === 'fertilizer' && (
              <div className="absolute top-6 sm:top-8 left-1/2 -translate-x-1/2 z-30 animate-[fadeIn_0.2s_ease-out]">
                <img src="/images/fertilizer.png" alt="영양제 주기" className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 object-contain drop-shadow-lg" />
              </div>
            )}
            ...
            <div className={`relative w-32 h-32 sm:w-40 sm:h-40 md:w-56 md:h-56 z-10 transition-transform duration-300 transform-gpu ${getCharacterAnimation()}`}>
              {isEvolved && evolutionId ? (
                <img src={`/images/evolutions/final-${evolutionId}.png`} alt="밤콩 최종 진화체" className="w-full h-full object-contain drop-shadow-lg transition-opacity duration-300 animate-fadeIn" />
              ) : (
                <img ref={characterRef} src={`/images/characters/level-${Math.min(Math.floor(displayLevel / 10) + 1, 10)}.png`} alt="밤콩 캐릭터" className="w-full h-full object-contain drop-shadow-lg transition-opacity duration-300 animate-fadeIn" />
              )}
            </div>
          </div>
          ...
          {level < 100 || isEvolved ? (
            <div className="flex w-full max-w-xl justify-center gap-2 sm:gap-3 md:gap-5">
              <button onClick={() => handleAction('water', 10, 1)} disabled={ap < 1 || level >= 200} className={`group relative flex-1 flex flex-col items-center justify-center gap-1 sm:gap-2 p-3 sm:p-4 md:p-5 rounded-[1.2rem] sm:rounded-[1.5rem] shadow-md border-2 transition-all overflow-hidden ${ap < 1 || level >= 200 ? 'bg-stone-100 border-stone-200 text-stone-400 cursor-not-allowed opacity-80' : 'bg-blue-50/80 backdrop-blur-sm hover:bg-blue-100 text-blue-600 border-blue-200 hover:-translate-y-1'}`}>
                <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 flex items-center gap-0.5 bg-white/90 px-1.5 sm:px-2 py-0.5 rounded-full border border-blue-100 text-[9px] sm:text-[11px] font-black shadow-sm"><Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-500 fill-amber-300" /> 1</div>
                <Droplet className={`w-6 h-6 sm:w-8 sm:h-8 ${ap >= 1 && level < 200 && 'group-active:scale-75 transition-transform'}`} />
                <span className="font-black text-xs sm:text-sm tracking-tighter">물 주기 (+10)</span>
              </button>
              <button onClick={() => handleAction('sun', 22, 2)} disabled={ap < 2 || level >= 200} className={`group relative flex-1 flex flex-col items-center justify-center gap-1 sm:gap-2 p-3 sm:p-4 md:p-5 rounded-[1.2rem] sm:rounded-[1.5rem] shadow-md border-2 transition-all overflow-hidden ${ap < 2 || level >= 200 ? 'bg-stone-100 border-stone-200 text-stone-400 cursor-not-allowed opacity-80' : 'bg-orange-50/80 backdrop-blur-sm hover:bg-orange-100 text-orange-600 border-orange-200 hover:-translate-y-1'}`}>
                <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 flex items-center gap-0.5 bg-white/90 px-1.5 sm:px-2 py-0.5 rounded-full border border-orange-100 text-[9px] sm:text-[11px] font-black shadow-sm"><Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-500 fill-amber-300" /> 2</div>
                <Sun className={`w-6 h-6 sm:w-8 sm:h-8 ${ap >= 2 && level < 200 && 'group-active:scale-75 transition-transform'}`} />
                <span className="font-black text-xs sm:text-sm tracking-tighter">햇빛 쬐기 (+22)</span>
              </button>
              <button onClick={() => handleAction('fertilizer', 35, 3)} disabled={ap < 3 || level >= 200} className={`group relative flex-1 flex flex-col items-center justify-center gap-1 sm:gap-2 p-3 sm:p-4 md:p-5 rounded-[1.2rem] sm:rounded-[1.5rem] shadow-md border-2 transition-all overflow-hidden ${ap < 3 || level >= 200 ? 'bg-stone-100 border-stone-200 text-stone-400 cursor-not-allowed opacity-80' : 'bg-pink-50/80 backdrop-blur-sm hover:bg-pink-100 text-pink-600 border-pink-200 hover:-translate-y-1'}`}>
                <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 flex items-center gap-0.5 bg-white/90 px-1.5 sm:px-2 py-0.5 rounded-full border border-pink-100 text-[9px] sm:text-[11px] font-black shadow-sm"><Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-500 fill-amber-300" /> 3</div>
                <Heart className={`w-6 h-6 sm:w-8 sm:h-8 ${ap >= 3 && level < 200 && 'group-active:scale-75 transition-transform'}`} />
                <span className="font-black text-xs sm:text-sm tracking-tighter">영양제 (+35)</span>
              </button>
            </div>
          ) : (
            <div className="flex w-full max-w-xl justify-center">
              <button onClick={() => setIsEvolutionModalOpen(true)} className="group relative w-full flex items-center justify-center gap-3 p-4 sm:p-5 rounded-[1.2rem] sm:rounded-[1.5rem] bg-gradient-to-r from-purple-500 via-fuchsia-500 to-pink-500 text-white shadow-[0_8px_0_0_#86198f,0_15px_20px_rgba(217,70,239,0.4)] hover:translate-y-1 active:translate-y-2 active:shadow-none transition-all duration-200 overflow-hidden">
                <Sparkles className="w-6 h-6 sm:w-8 sm:h-8 animate-pulse" />
                <span className="font-black text-lg sm:text-xl drop-shadow-md">밤콩이 최종 진화시키기</span>
        ...
        <div className="w-full lg:w-[400px] flex flex-col gap-4 sm:gap-6 shrink-0 relative z-10 mt-4 lg:mt-0">
          ...
          <div className="flex w-full gap-3 sm:gap-4">
            ...
            <Link
              href="/minigames"
              className="group relative flex-1 flex flex-col items-center justify-center gap-1.5 sm:gap-2 p-3 sm:p-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-[1.2rem] sm:rounded-[1.5rem] shadow-[0_6px_0_0_#92400e,0_10px_15px_rgba(245,158,11,0.3)] sm:shadow-[0_8px_0_0_#92400e,0_15px_20px_rgba(245,158,11,0.3)] hover:translate-y-1 active:translate-y-2 active:shadow-none transition-all duration-200 border border-orange-400/50"
            >
              <div className="absolute -top-3 sm:-top-4 left-1/2 -translate-x-1/2 w-max bg-white text-orange-600 px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-[11px] font-black shadow-md border border-orange-200 animate-bounce">
                행동력이 부족하다면?
              </div>
              <Gamepad2 className="w-6 h-6 sm:w-7 sm:h-7 group-hover:rotate-12 transition-transform" />
              <span className="font-black text-sm sm:text-base drop-shadow-sm leading-tight text-center break-keep">
                미니게임 {level < 100 ? '하기' : ''}
              </span>
            </Link>

            {/* 2. 내 방 가기 버튼 (100레벨 이상 해금) */}
            {level >= 100 && (
              <Link
                href="/game/room"
                className="group relative flex-1 flex flex-col items-center justify-center gap-1.5 sm:gap-2 p-3 sm:p-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-[1.2rem] sm:rounded-[1.5rem] shadow-[0_6px_0_0_#86198f,0_10px_15px_rgba(217,70,239,0.3)] sm:shadow-[0_8px_0_0_#86198f,0_15px_20px_rgba(217,70,239,0.3)] hover:translate-y-1 active:translate-y-2 active:shadow-none transition-all duration-200 border border-pink-400/50 overflow-hidden"
              >
                <div className="absolute inset-0 bg-white/20 -translate-x-full group-hover:animate-[shimmer_1s_infinite]"></div>
                <Home className="w-6 h-6 sm:w-7 sm:h-7 group-hover:-translate-y-1 transition-transform" />
                <span className="font-black text-sm sm:text-base drop-shadow-sm leading-tight text-center break-keep">
                  내 밤콩이 방
                </span>
              </Link>
      ...
      {/* 분리된 모달 컴포넌트 마운트 */}
      <RewardModal isOpen={isRewardModalOpen} onClose={() => setIsRewardModalOpen(false)} level={level} />
      <EvolutionModal isOpen={isEvolutionModalOpen} saveEvolution={saveEvolution} />
    </div>
  );
}